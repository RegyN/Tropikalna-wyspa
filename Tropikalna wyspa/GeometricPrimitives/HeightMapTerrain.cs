        ...
        public HeightMapTerrain(GraphicsDevice graphicsDevice, float size, float[][] heightMap, Vector3[][] normalMap)
        {
            for (int i = 0; i < heightMap.Length - 1; i++)
            {
                for (int j = 0; j < heightMap[0].Length - 1; j++)
                {
                    ...
                    Vector3 sideLeft = new Vector3(size, heightMap[i + 1][j] - heightMap[i][j], 0f);
                    Vector3 sideRight = new Vector3(size, heightMap[i + 1][j + 1] - heightMap[i][j + 1], 0f);
                    Vector3 sideUp = new Vector3(0f, heightMap[i][j + 1] - heightMap[i][j], size);
                    Vector3 sideDown = new Vector3(0f, heightMap[i + 1][j + 1] - heightMap[i + 1][j], size);
                    ...
                    Vector3 upperLeftPos = new Vector3(i*size, heightMap[i][j], j*size);

                    AddVertex(upperLeftPos, normalMap[i][j], new Vector2(0.0f, 0.0f));
                    AddVertex(upperLeftPos + sideLeft, normalMap[i + 1][j], new Vector2(1.0f, 0.0f));
                    AddVertex(upperLeftPos + sideLeft + sideDown, normalMap[i + 1][j + 1], new Vector2(1.0f, 1.0f));
                    AddVertex(upperLeftPos, normalMap[i][j], new Vector2(0.0f, 0.0f));
                    AddVertex(upperLeftPos + sideUp + sideRight, normalMap[i + 1][j + 1], new Vector2(1.0f, 1.0f));
                    AddVertex(upperLeftPos + sideUp, normalMap[i][j + 1], new Vector2(0.0f, 1.0f));
                }
